package com.yuchen.streamingRecommend
...
object StreamingRecommend {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("StreamingRecommend").setMaster(config("spark.cores"))
      .set("spark.excutor.memory","6G").set("spark.driver.memory","2G")
    //创建SparkSession
      ...
      rdd =>
        rdd.map{
          case (uid,mid,score,timestamp) =>
            ...
            //获取电影p最相似的k个电影
            val simMovies = getTopSimMovies(MAX_SIM_MOVIES_NUM,mid,uid,simMoviesMartrixBroadCast.value)

            //计算待选电影的推荐优先级
            val streamRecs = computeMovieScores(simMoviesMartrixBroadCast.value,userRecentlyRatings,simMovies)
  ...
  def saveRecsToMongoDB(uid: Int, streamRecs: Array[(Int,Double)])(implicit mongoConfig: MongoConfig): Unit ={
    //连接StreamRecs
    val streamingRecsCollection = ConnHelper.mongoClient(mongoConfig.db)(MONGO_STREAM_RECS_COLLECTION)
  ...
  def getTopSimMovies(num: Int, mid: Int, uid: Int, simMovies: scala.collection.Map[Int, scala.collection.immutable.Map[Int, Double]])(implicit mongoConfig: MongoConfig) : Array[Int] = {
    ...
    //获取用户已经观看过的电影
    val ratingExist = ConnHelper.mongoClient(MONGO_DATABASE)(MONGO_RATING_COLLECTION).find(MongoDBObject("uid" -> uid)).toArray.map { item =>
...
object ConnHelper extends Serializable{
  ...
  lazy val mongoClient = MongoClient(MongoClientURI("mongodb://yuchen:27017/"+MONGO_DATABASE))
}
